...
class UnionFind:
    ...
    def count_circuits(self) -> int:
        """
        Count the number of distinct electrical circuits.

        Returns:
            Number of separate circuits currently formed.
        """
...
def solve_part2(junction_boxes: list[tuple[int, int, int]]) -> int:
    """
    Connect junction boxes until all form a single circuit, tracking the final connection.

    Continues processing the closest pairs of junction boxes by distance until all
    junction boxes are connected in a single electrical circuit. The goal is to find
    the last connection needed to merge everything into one circuit.

    Args:
        junction_boxes: List of junction box positions as (x, y, z) tuples.

    Returns:
        Product of the X coordinates of the last two junction boxes that needed
        to be connected to form a single circuit.
    """
    ...
    print(f"Connecting junction boxes until single circuit formed...")
    last_i: int = -1
    last_j: int = -1
    ...
    for dist, i, j in distances:
        if uf.union(i, j):
            last_i = i
            last_j = j
            # Check if we now have a single circuit
            if uf.count_circuits() == 1:
                print(f"Single circuit formed! Last connection: boxes {i} and {j}")
                break

    # Get the X coordinates of the last two junction boxes connected
    x1: int = junction_boxes[last_i][0]
    x2: int = junction_boxes[last_j][0]
...
def solve(
    junction_boxes: list[tuple[int, int, int]], num_connections: int = 1000
) -> int:
    """
