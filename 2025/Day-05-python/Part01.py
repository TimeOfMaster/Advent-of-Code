...
def is_availble_also_fresh(available: int, fresh_ingredients: list[tuple[int, int]]) -> bool:
    for start, end in fresh_ingredients:
        if start <= available <= end:
            return True
    return False

def main(fresh_ingredients: list[tuple[int, int]], available_ingredients: list[int]) -> int:
